...
// @ts-ignore
import { getIdFromName } from "./fuzzy-dict.js";

wtf.extend(wtfPluginApi);
const wtfOpts = { domain: "brawlstars.fandom.com", path: "api.php" };

export const brawlerId = ({ name }: { name: string }) => name.replace(/[. &]/g, "_").toLowerCase();
...
export interface Hypercharge extends DescribeAble, StatsAble, AssetAble {}
...
export class FandomScraper {
	...
	async getModeData(modeName: string): Promise<FandomModeData | undefined> {
		...
		const history: HistoryEntry[] = (
			...
			.map((l: any) => {
				...
				const date = new Date(Date.parse(`20${groups[3]}-${groups[2]}-${groups[1]}`));
	...
	private parseDescriptions(brawlerPage: any, _$: cheerio.CheerioAPI) {
	...
	private parseStats(brawlerPage: any, brawlerName: string) {
		const infobox = (brawlerPage.infobox("brawler infobox") ?? brawlerPage.infobox(`${brawlerName} infobox`)).json();
	...
	private parseHealthByLevel(brawlerPage: any, brawlerName: string): Pick<FandomBrawlerData, "healthByLevel"> {
		const infobox = brawlerPage.infobox("brawler infobox") ?? brawlerPage.infobox(`${brawlerName} infobox`);

		const generateStatsPerLevelList = (base: number) => {
			const statsPerLevel: number[] = [];
		...
		return {
			healthByLevel: generateStatsPerLevelList(infobox.get("health").json().number),
		};
	}
	...
	private parseAttack(
		...
		$: cheerio.CheerioAPI,
		brawlerName?: string
	): Pick<FandomBrawlerData, "attack" | "altAttack"> {
		...
		if (brawlerName === "Kaze") {
			const res = {
				attack: undefined as Attack | undefined,
				altAttack: undefined as Attack | undefined,
			};
			for (const [idx, attackSection] of attackSections.entries() as any[]) {
				const title = attackSection.title();
				const info = this.parseInfoboxTable($, `${title} (Attack)`);
				...
				if (idx === 0) {
					res.attack = {
						...
						description,
						stats: info?.stats ?? [],
						statsByLevel: info?.statsByLevel ?? [],
					};
				} else {
					res.altAttack = {
						name,
			...
			return res as Pick<FandomBrawlerData, "attack" | "altAttack">;
		} else {
	...
	private parseSuper(
		...
		$: cheerio.CheerioAPI,
		brawlerName?: string
	): Pick<FandomBrawlerData, "super" | "altSuper"> {
		...
		if (brawlerName === "Kaze") {
			const res = {
				super: undefined as Attack | undefined,
				altSuper: undefined as Attack | undefined,
			};
			for (const [idx, attackSection] of superSections.entries() as any[]) {
				const title = attackSection.title();
				const info = this.parseInfoboxTable(
					$,
					`${title.replace("Fist of the Brawl Star", "Fist of the North Star")} (Super)`
				);

				const { name, description } = this.parseNameAndDescription(attackSection, "super");
				if (idx === 0) {
					res.super = {
						name,
						description,
			...
			return res as Pick<FandomBrawlerData, "super" | "altSuper">;
		} else {
	...
	private findSectionOrSections(brawlerPage: any, startsWith: string) {
		const section = brawlerPage.sections().find((s: any) => s.title().startsWith(startsWith));
	...
	private parseHypercharge(
		brawlernName: string,
		brawlerPage: any,
		$: cheerio.CheerioAPI
	): Pick<FandomBrawlerData, "hypercharge"> {
		...
		return {
			hypercharge: {
				...
				asset: this.findAssetByAttr($, `${brawlernName}-Hypercharge`, "data-image-name")!,
			},
		};
	}
	...
	private parseGadgets(
		...
		const fixedName =
			brawlerName === "Mr. P" ? brawlerName.replaceAll(" ", "") : brawlerName === "Jae-Yong" ? "Jae-yong" : brawlerName;
		...
		for (let gadgetIndex = 0; gadgetIndex < gadgetSections.length; gadgetIndex++) {
			...
			const id = getIdFromName(name, gds);

			gadgets.push({
				name,
				id: id.id.toString(),
				description,
				asset: this.findAssetByAttr($, `GD-${fixedName}${gadgetIndex + 1}`, "data-image-name")!,
				statsByLevel: gadgetStats?.statsByLevel ?? [],
				...(gadgetStats?.stats?.[0] ? { cooldown: gadgetStats?.stats?.[0].value ?? "Unknown" } : {}),
			});
		}
	...
	private parseStarpowers(
		...
		for (let starpowerIndex = 0; starpowerIndex < starpowerSections.length; starpowerIndex++) {
			...
			const id = getIdFromName(name, sps);

			starpowers.push({
				name,
				id: id.id.toString(),
				description,
				asset: this.findAssetByAttr($, `SP-${fixedName}${starpowerIndex + 1}`, "data-image-name")!,
	...
	private parseInfoboxTable(
		...
		const statsByLevel = names
			.map((name, columnIndex) => ({
				name,
				levels,
				values: values.filter((v) => v.columnIndex == columnIndex).map((v) => v.value),
			}))
	...
	private parseVoicelines(brawlerPage: any, $: cheerio.CheerioAPI): { skinName: string; voiceline: Voiceline }[] {
		if (brawlerPage.section("Voice Lines") === null) return [];
		const voicelinesWikitext = (brawlerPage.section("Voice Lines").table(0).wikitext() as string).split("\n");
		...
		const advance = () => {
			if (voiceline.trigger != undefined && voiceline.description != undefined && voiceline.asset != undefined) {
				rows.push({
					skinName,
					voiceline: {
		...
		// 					trigger: headers[columnIndex],
		// 				};
		// 				skinName = part.substring(0, part.length - 1).replace("Normal", "Default");
		// 			}
		// 		}
		// 	}
		// }
	...
	private findAssetByAttr($: cheerio.CheerioAPI, value: string, attr: "title" | "alt" | "data-image-name"): Asset | undefined {
		const img = $("img")
			.filter((i, el) => {
				const titleText = $(el).attr(attr) ?? "";
				console.log("checking img", { titleText, value, attr });
				return titleText.includes(value);
			})
			.first();
		console.log(img.attr(), img.html());
	...
	private parseImgAsAsset(img: cheerio.Cheerio<any>): Asset | undefined {
		...
		const sourceUrl = src?.split("/smart")[0]?.split("/scale")[0] + (queryParams ? "?" + queryParams : ""); // remove resizing parameters
	...
	public parseEnumeratedKeys(template: any): any[] {
		...
		keys.forEach((key) => {
			const match =
				key.match(/^(\w+?)(p+)(\d+)(?:_(\d+))?$/) ?? key.match(/^(\w+?)(p)()(?:_(\d+))?$/) ?? key.match(/^(\w+?)(\d+)?$/);
			if (match == null) {
				return;
			}
			...
			const index1 = match[3] != "" ? Number.parseInt(match[3] ?? match[2]) : 0;
			const index2 = match[4] != undefined ? Number.parseInt(match[4]) - 1 : 0;
	...
	private parseSprays($: cheerio.CheerioAPI): { skinName: string; spray: Spray }[] {
	...
	private parseProfileIcons($: cheerio.CheerioAPI): { skinName: string; profileIcon: Asset }[] {
	...
	private parseSkins(brawlerName: string, brawlerPage: any, $: cheerio.CheerioAPI): Pick<FandomBrawlerData, "skins"> {
		const skinTemplates: any[] = brawlerPage.templates("skin gallery").map((t: any) => t.json());
		...
		const popPinsFor = (skin: Asset) => popMatching(pins, (p) => p.skin.filename == skin.filename).map((p) => p.pin);

		const voicelines = this.parseVoicelines(brawlerPage, $);
		const popVoicelinesFor = (skinName: string) =>
			popMatching(voicelines, (v) => v.skinName == skinName).map((v) => v.voiceline);
		...
		const matchSkinName =
			...
			({ skinName: actualSkinName }: { skinName: string }) => {
				...
				const cleanedActualSkinName = actualSkinName.replace(brawlerName, "").trim();
		...
		for (let i = 0; i < skinTemplates.length; i++) {
			...
			const addSkin = (skin: Omit<Skin, "asset" | "petSkins" | "pins" | "voicelines" | "sprays" | "profileIcons">) => {
				const asset = this.findAssetByAttr(
					$,
					brawlerName + (template.for ? " " + template.for : " Skin") + "-" + skin.name + ".png",
					"title"
				);
			...
			for (let j = 0; j < enumeratedKeys.length; j++) {
				...
				console.log("enumerated key: ", e);
				if (!e.name) {
					console.warn("Skin template has enumerated keys but no name", { template, enumeratedKeys });
					continue;
				}
				addSkin({
					name: e.name.join(", "),
					...
					cost: e.cost ? e.cost.join(", ").replaceAll(" • E", "").replaceAll(" • S", "") : undefined,
		...
		if (pins.length > 0) {
			console.warn("Could not match all pins to skins", pins.length);
		}

		if (voicelines.length > 0) {
			console.warn("Could not match all voicelines to skins", voicelines.length);
		}

		if (sprays.length > 0) {
			console.warn("Could not match all sprays to skins", sprays.length);
		}

		if (profileIcons.length > 0) {
			console.warn("Could not match all profile icons to skins", profileIcons.length);
		}
	...
	private parseBalanceHistory(brawlerPage: any): Pick<FandomBrawlerData, "history"> {
		// 			writeFileSync("test.json", JSON.stringify(parse(brawlerPage.wikitext()), null, 2));
		try {
			const balanceHistory = brawlerPage
				...
				.lines()
				.map((l: any) => l.text()) as string[];
			if (balanceHistory.every((s) => s.endsWith(":"))) {
				const parsed = parse(brawlerPage.wikitext());
				const historyFirstThingIndex = parsed.findIndex(
					(s: any) => s.type === "heading" && s.content && Array.isArray(s.content) && s.content[0] === "History"
				);
				const list = parsed[historyFirstThingIndex + 1].items;

				const history: BalanceHistoryEntry[] = [];
				let date: Date | undefined = undefined;

				for (const line of list) {
					if (line.level === 1 && line.content[0].endsWith(":")) {
						const groups = line.content[0].match(/^(\d{2})\/(\d{2})\/(\d{2}):$/);
						if (groups == null) {
							console.warn("Could not parse date", line.content[0]);
							continue;
						}
						date = new Date(Date.parse(`20${groups[3]}-${groups[2]}-${groups[1]}`));
					}
					if (date != undefined && line.level == 2) {
						const kind = line.content[0].parameters.change[0];
						const description = nodeToText(line.content[0].parameters);
			...
			console.warn("Couldn't parse history for " + brawlerPage.title());
			return {
				history: [],
			};
		}
	...
	async getBrawlerData(
	...
	): Promise<FandomBrawlerData | undefined | "skipped"> {
		const brawlerPage: any = await wtf.fetch(brawlerName, wtfOpts);
		console.log("Fetching data for", brawlerName);
		if (!brawlerPage) {
			console.log("Brawler page not found for", brawlerName, " skipping...");
			return "skipped";
		}
		const url = brawlerPage.url().replace("//en.", "//");
		const data = await fetch(
			`https://brawlstars.fandom.com/api.php?action=parse&page=${encodeURIComponent(brawlerPage.title())}&format=json`,
			{}
		);
		...
		if (brawlerName === "Buzz Lightyear") {
			return;
		}

		if (brawlerName === "Jae-Yong") {
		...
		return {
			...
			...this.parseAttack(brawlerPage, $, brawlerName),
			...this.parseSuper(brawlerPage, $, brawlerName),
			...this.parseHypercharge(brawlerName, brawlerPage, $),
			...this.parseHealthByLevel(brawlerPage, brawlerName),
			...this.parseStats(brawlerPage, brawlerName),
...
//   .getBrawlerData("Bea")
//   .then(
//     async (data) => await writeFile("bea.json", JSON.stringify(data, null, 2))
//   );
