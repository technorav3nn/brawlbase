...
import { listProfiles } from "@brawlbase/supercell-id-api";
import { getCdnUrlForAvatarId, idToHighLow, tagToId } from "@brawlbase/supercell-id-api/browser";
...
export default defineEventHandler(async (event) => {
	...
	if (existingUser) {
		throw createError({
			status: 409,
			statusMessage: "User with tag already exists, only one account can have one player tag",
		});
	}
