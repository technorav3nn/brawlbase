	...
	<USlideover v-if="user" :ui="{ body: 'overflow-auto! p-0!', content: 'max-w-sm', header: 'min-h-[4rem]! p-0 px-4!' }">
		...
		<template #body>
			...
			<TransitionGroup
				...
				tag="div"
				:style="{
					'--transform-data': `translateX(${
						computeTabsDirection() === 'none' ? 0 : computeTabsDirection() === 'right' ? 18 : -18
					}px)`,
				}"
			>
				...
				<div v-if="tab === 'settings'">
					<div class="w-full border-b border-neutral-200 px-4 py-5 dark:border-neutral-800">
						...
						<p class="text-sm text-neutral-500 dark:text-neutral-400">
							Edit everything about your account, or logout of it
						</p>
					</div>
					<div class="mt-2 flex flex-col gap-2 px-2">
...
.tabs-leave-active {
	width: 100%;
	position: absolute;
	transition: transform 0.212s cubic-bezier(0.5, 0.7, 0.1, 1), opacity 0.1s cubic-bezier(0.5, 0.7, 0.1, 1);
}
.tabs-enter-from,
.tabs-leave-to {
	width: 100%;
